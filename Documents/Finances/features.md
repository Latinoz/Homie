...
- **Количество из журнала операций**: при добавлении актива с количеством автоматически создаётся операция «Покупка криптовалюты»; последующие покупки/продажи фиксируются в журнале и пересчитывают количество актива (средневзвешенная цена)
- **Кошельки и биржи**: актив привязывается к финансовому счёту типов `Wallet` (криптокошелёк) и `CryptoExchange` (биржа). Для криптокошельков из справочника `Wallets` (тип Crypto) связанный счёт создаётся автоматически

### Поля формы

- Инструмент (из справочника, тип «Криптовалюта»)
- Тикер (BTC, ETH и др.)
- CoinGecko ID (для API)
- Кошелёк / Биржа (список криптокошельков и криптобирж)
- Дата покупки
- Количество (при > 0 создаётся операция покупки в журнале)
- Валюта учёта
- Средняя цена покупки
...
**Кошельки:** у кошелька есть тип (Обычный / Криптокошелёк). Для криптокошелька автоматически создаётся связанный финансовый счёт; на странице списка отображаются остатки криптовалюты по монетам и стоимость в ₽. Форма создания поддерживает `returnUrl` для возврата на страницу Crypto/Create.
